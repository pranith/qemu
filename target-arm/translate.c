...
{                                                                           \
    ...
    if (qsim_gen_callbacks) {                                               \
      ...
      gen_helper_load_callback_pre(cpu_env, a32, tmp_size, tmp_type);                \
    }                                                                       \
    gen_aa32_ld_i32(s, val, a32, index, OPC | s->be_data);                  \
    if (qsim_gen_callbacks) {                                               \
      gen_helper_load_callback_post(cpu_env, a32, tmp_size, tmp_type);               \
...
{                                                                        \
    ...
    if (qsim_gen_callbacks) {                                            \
      ...
      gen_helper_store_callback_pre(cpu_env, a32, tmp_size, tmp_type);            \
    }                                                                    \
    gen_aa32_st_i32(s, val, a32, index, OPC | s->be_data);               \
    if (qsim_gen_callbacks) {                                            \
      gen_helper_store_callback_post(cpu_env, a32, tmp_size, tmp_type);           \
      tcg_temp_free_i32(tmp_size);                                       \
      tcg_temp_free_i32(tmp_type);                                       \
    }                                                                    \
}
